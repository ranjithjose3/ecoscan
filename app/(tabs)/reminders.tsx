...
function ymdToday() { return fmt(new Date()); }
...
export default function ReminderScreen() {
  ...
  const scheduleReminder = async () => {
    ...
    await Notifications.scheduleNotificationAsync({
      content: { title: 'Reminder 📌', body: 'This is your scheduled reminder (10 sec after now)!' },
      trigger,
    });
  };
  ...
  const confirmRemove = async () => {
    ...
    try {
      ...
      remindersBus.emit({ event_id: row.event_id, place_id: row.place_id, hasReminder: false });
    } catch (e) {
  ...
  const renderItem = ({ item }: { item: ReminderRow }) => {
    const title = item.event_title || 'Reminder';
        ...
        <View style={styles.row}>
          ...
          <View style={styles.leftCol}>
            <View style={styles.topLine}>
              <Chip
                compact
                style={[styles.dateChip, { backgroundColor: theme.colors.brandContainer }]}
                textStyle={{
                  color: theme.colors.onBrandContainer,
                  fontSize: 11,
                  lineHeight: 13,
                  fontWeight: '700',
                }}
  ...
  const listBottomPadding = Math.max(24, insets.bottom + tabBarHeight + 16);
  ...
  return (
    <>
      <ScreenLayout 
        ...
        <Card.Actions>
          <Button mode="contained" onPress={scheduleReminder}>Schedule Reminder (10 sec)</Button>
        </Card.Actions>
        ...
        {reminders.length === 0 ? (
          ...
          <FlatList
            ...
            ListFooterComponent={<View style={{ height: tabBarHeight + insets.bottom + 16 }} />}
            removeClippedSubviews={false}
          />
        )}
      </ScreenLayout>

      {/* Confirmation dialog (keeps theme surface background) */}
      <Portal>
        <Dialog visible={confirmOpen} onDismiss={closeConfirm} style={styles.dialog}
        theme={{ colors: { backdrop: 'rgba(0,0,0,0.72)' } }}
        >{/* 
          <Dialog.Icon icon="bell-off-outline" /> */}
              ...
              <>
                <View style={styles.confirmTopLine}>
                  <Chip
                    ...
                    textStyle={{ fontSize: 11, lineHeight: 13, fontWeight: '700' }}
                  >
                    {confirmRow.event_date}
                  </Chip>
                  <Text style={styles.confirmTitle} numberOfLines={2}>
                    {confirmRow.event_title || 'Reminder'}
                  </Text>
                </View>
          ...
          <Dialog.Actions style={styles.actions}>
            <View style={styles.actionsRow}>
              <Button mode="outlined" icon="close" onPress={closeConfirm} style={[styles.actionBtn, styles.cancelBtn]} contentStyle={styles.actionContent}>
                Cancel
              </Button>
              <Button mode="contained" icon="check" onPress={confirmRemove} style={[styles.actionBtn, styles.removeBtnPill]} contentStyle={styles.actionContent}>
                Remove
              </Button>
            </View>
          </Dialog.Actions>
...
const styles = StyleSheet.create({
  ...
  confirmTitle: { fontSize: 16, fontWeight: '700', flexShrink: 1 },
